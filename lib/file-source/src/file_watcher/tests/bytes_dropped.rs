...
use crate::{file_watcher::FileWatcher, ReadFrom};

/// Test that get_bytes_dropped() returns accurate values
#[test]
fn test_bytes_dropped_basic() {
...
#[test]
fn test_bytes_dropped_after_delete() {
    let dir = tempfile::TempDir::new().expect("could not create tempdir");
    let path = dir.path().join("test.log");
...
#[test]
fn test_bytes_dropped_growing_file() {
    let dir = tempfile::TempDir::new().expect("could not create tempdir");
    let path = dir.path().join("test.log");
    ...
    // since we use current file size, not initial
    assert_eq!(fw.get_bytes_dropped(), 12);
