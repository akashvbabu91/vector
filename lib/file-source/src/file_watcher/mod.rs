...
impl FileWatcher {
    ...
    pub async fn update_path(&mut self, path: PathBuf) -> io::Result<Option<FileUnwatchInfo>> {
        ...
        let (new_devno, new_inode) = (file_info.portable_dev(), file_info.portable_ino());
        ...
        let unwatch_info = if (new_devno, new_inode) != (self.devno, self.inode) {
            ...
            let new_metadata = file_handle.metadata().await?;
            let new_file_size = new_metadata.len();

            let (new_reader, new_position): (Box<dyn AsyncBufRead + Send + Unpin>, FilePosition) = if gzipped {
